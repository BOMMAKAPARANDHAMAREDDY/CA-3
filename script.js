window.onload = function () {
    ...
    fetch('https://www.themealdb.com/api/json/v1/1/random.php')
        ...
        .then(data => {
            ...
            const html = `
            <img src="${meal.strMealThumb}" alt="${meal.strMeal}" onclick="listOfIngredients('${meal.idMeal}')">
            <h2>${meal.strMeal}</h2>
            `;
    ...
    function displayData(inputData) {
        ...
        if (inputData.meals) {
            inputData.meals.forEach(meal => {
                container.innerHTML += `<img src="${meal.strMealThumb}" alt="recipe image" onclick="listOfIngredients('${meal.idMeal}')">
                <h2>${meal.strMeal}</h2>`;
            });
        } else {
...
function listOfIngredients(id) {
    fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${id}`)
        .then(res => res.json())
        .then(list => {
            let meal = list.meals[0]
            // console.log(meal)
